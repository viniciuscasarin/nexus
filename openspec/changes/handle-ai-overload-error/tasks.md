...
- [ ] 2.2 Update `src/components/job-analysis.tsx` to intercept `"AI_MODEL_OVERLOADED"` in the `analyzeJob` catch block (if applicable). Verify the toast is shown.
- [ ] 2.3 Update `src/components/job-application-details.tsx` to intercept `"AI_MODEL_OVERLOADED"` in `handleGenerate`. Trigger a `toast.error` (adding `sonner` if not imported) with a Retry action. Verify by manually throwing the error and checking the toast appearance.
